...
public class SimpleHeatmap : MonoBehaviour
{
    ...
    // Parameters for viewpoint sampling
    ...
    void Start()
    {
        // Initialize heatmap and supporting data structures
        ...
        // Identify obstacles, sample visibility points, and build the grid
        GenerateBlockers();
        GenerateViewPoints();
        CreateGrid();

        // Begin the main loop: compute heatmap and move the robot
        StartCoroutine(UpdateHeatmapAndNavigate());
    }

    // Finds and stores obstacles (chairs, desks) for occlusion handling
    ...
    // Randomly samples 3D points visible from the robot's camera
    ...
    // Instantiates a grid of cells representing the floor
    void CreateGrid()
    {
    ...
    // Continuously updates heatmap and moves robot along A* path
    IEnumerator UpdateHeatmapAndNavigate()
    {
        while (true)
        {
            // Recalculate exposure scores
            ComputeHeatmap();

            // Plan a new path to the target based on current heatmap
            ...
            if (path != null && path.Count > 1)
            {
                for (int i = 1; i < path.Count; i++)
                {
                    ...
                    // Interpolate robot movement smoothly to next cell
                    ...
                    // Mark cells as visited (blue) only if they were visible
    ...
    // Computes visibility-based exposure score for each grid cell
    void ComputeHeatmap()
    {
        for (int x = 0; x < rows; x++)
        {
            for (int z = 0; z < cols; z++)
            {
                ...
                // Interpolate cell color based on exposure score
    ...
    // Checks if a point is within the robot's camera view
    ...
    // A* pathfinding based on heatmap cost
